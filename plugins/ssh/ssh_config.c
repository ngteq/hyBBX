...
void hybbx_ssh_config_defaults(hybbx_ssh_config_t *config)
{
    ...
    hybbx_strlcpy(config->bind_v4, HYBBX_SSH_DEFAULT_BIND_V4,
                  sizeof(config->bind_v4));
    hybbx_strlcpy(config->bind_v6, HYBBX_SSH_DEFAULT_BIND_V6,
                  sizeof(config->bind_v6));
    hybbx_strlcpy(config->hostkey_dir, HYBBX_SSH_DEFAULT_HOSTKEY_DIR,
                  sizeof(config->hostkey_dir));
    config->port = HYBBX_SSH_DEFAULT_PORT;
    config->ipv4 = 1;
    config->ipv6 = 1;
}
...
{
    char scratch[HYBBX_SSH_BIND_V6_MAX];
    const char *value;
    ...
    value = find_kv(config, "hostkey_dir", scratch, sizeof(scratch));
    if (value != NULL && value[0] != '\0') {
        hybbx_strlcpy(out->hostkey_dir, value, sizeof(out->hostkey_dir));
    }

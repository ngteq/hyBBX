...
C99 multi-transport session daemon (mail/chat/commands + link adapters). **Main** + **Secondary** topology; default Main: TCP/IP + HBX (`ax25=no`). **0.7.x** line; `RELEASE-*` docs from **v1.0.0**. INI: `share/hybbx.ini.example`, `share/hybbx-secondary.ini.example`.
...
4. Packet radio bridges RF ↔ HBX/TCP (`circuit_host` / `[circuit]` hub).
